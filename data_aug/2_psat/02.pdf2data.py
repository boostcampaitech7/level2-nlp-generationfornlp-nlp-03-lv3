    ...
    for element in page_layout:
        if isinstance(element, LTTextContainer):
            ...
            if side == "left" and element.x0 < page_layout.width / 2:
                # 왼쪽 페이지 상단 불필요 텍스트 제거, 연도에 맞춰서 수정 필요
                text = re.sub(r"2022년도\s+국가공무원\s+5급\s+공채\s+등\s+필기시험", "", text)
                elements.append(re.sub(r"\n", "", text))
            elif side == "right" and element.x0 >= page_layout.width / 2:
                # 오른쪽 페이지 상단 불필요 텍스트 제거
                text = re.sub(r"언어논리영역", "", text)
                text = re.sub(r"나 책형", "", text)
...
file_path = "../resources/aug/psat/2022/2. 언어논리(나).pdf"  # TO_DO 문제지 파일 명
...
ans_df = pd.read_csv("../resources/aug/psat/2022/answer.csv")  # TO_DO pdf2ans.py 결과물 파일명
...
total_data.to_csv(
    "../resources/aug/psat/2022/extract_from_pdf.csv", encoding="utf-8-sig"
)  # TO_DO 저장할 파일명,경로명
