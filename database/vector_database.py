...
class VectorDatabase(object):
    ...
    def _load_wikidata_by_chunk(self, wiki_path, num_sent=5, overlap=0, cpu_workers=None, gold_passages=None):
        ...
        with Pool(cpu_workers) as pool:
            file_paths = [
                os.path.join(wiki_path, subset, wiki_file)
        ...
        with Pool(cpu_workers) as pool:  # chunk_idx_results
            for txt_results, title_results, duplicates in tqdm(
                pool.imap_unordered(self._process_text, tasks), total=len(tasks)
            ):
    ...
    def encode_text(
        self, title, text, embedding_model, tokenizer, pooler=None, max_length=512, batch_size=32, device="cuda"
    ):
    ...
    def build_embedding(
        ...
        idx_lst, txt_lst, title_lst = self._load_wikidata_by_chunk(
            wiki_path, num_sent, overlap, cpu_workers, gold_passages
        )

        all_embeddings = self.encode_text(
            title_lst, txt_lst, embedding_model, tokenizer, pooler, max_length, batch_size, device
        )
