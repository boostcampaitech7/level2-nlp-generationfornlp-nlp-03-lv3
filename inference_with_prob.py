...
def inference_by_logit(model, dataset, raw_dataset, tokenizer):
    ...
    with torch.inference_mode():
        for i, data in tqdm(enumerate(dataset), total=len(dataset)):
            ...
            probs_dict = {f"prob_{i+1}": f"{prob:.6f}" for i, prob in enumerate(probs)}
            
            infer_results.append({"id": _id, "answer": predict_value, **probs_dict})

    pd.DataFrame(infer_results).to_csv("output/kfold_output_0_6468.csv", index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint", type=str, default="./resources/checkpoint/fold/0/checkpoint-6468")
