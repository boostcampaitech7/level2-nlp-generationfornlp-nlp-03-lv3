...
def main():
    ...
    train_dataset, eval_dataset = dm.get_processing_data(
        use_kfold=model_args.use_kfold,
        k_fold=model_args.k_fold, 
        fold_num=model_args.fold_num
    )
    ...
    # Training
    with mlflow.start_run(run_name=f"k fold_{model_args.fold_num}"):  # 실험 안 run name
        ...
        mlflow.transformers.log_model(
            ...
            registered_model_name=f"aug 1_fold_{model_args.fold_num}",  # 원하는 실험 이름으로 바꾸기.
        )
